...
This line is **archived**: its Angular major (18) left Angular's support window
(November 2025), so it receives no features, fixes, ports, or releases. The published npm
versions stay available:

```
npm install @artstesh/charts@7
```

npm serves this line under the `v7-latest` dist-tag. The actively maintained lines are
