...
This line is **archived**: its Angular major (16) left Angular's support window
(November 2024), so it receives no features, fixes, ports, or releases. The published npm
versions stay available:

```
npm install @artstesh/charts@5
```

npm serves this line under the `v5-latest` dist-tag. The actively maintained lines are
